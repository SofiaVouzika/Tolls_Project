...
def passes_cost(ar):
    url = 'http://localhost:8000/interoperability/api/passescost/' + ar.op1 + '/' + ar.op2 + '/' + str(ar.datefrom) + '/' + str(ar.dateto)
    if (ar.format == 'csv'):
        url = url + '?format=csv'
...
parser.add_argument('--op2', help = 'Choose unique second operator', required = 'TRUE')
